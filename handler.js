...
let kratos = `${plugins.botAdmin || plugins.admin || plugins.group || plugins || noPrefix || hl ||  m.text.slice(0, 1) == hl || plugins.command}`
if (adminMode && !isOwner && !isROwner && m.isGroup && !isAdmin && kratos) return   
...
this.reply(m.chat, ` *NO TIENE ATROCOINS*`, m)
...
if (m.money)
m.reply(+m.money + ' *ATROCOINS USADO(S)* 🇦🇱')
break
}}} catch (e) {
console.error(e)
} finally {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'KRATOS-SOLUTIONS-OFC 🇦🇱') :
...
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: lenguajeGB.smsAvisoAG().slice(0,-2), body: [wm, 🇦🇱' 𝗦𝘂𝗽𝗲𝗿 ' + KT + ' 🇦🇱', '🌟 clubatroofc.gmail.com'].getRandom(), thumbnail: kratroImg.getRandom(), sourceUrl: [md, yt, nna, nn, nnn, ig, paypal, fb].getRandom() }}}}, tg)
