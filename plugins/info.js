...
frep = { contextInfo: { externalAdReply: {title: wm, body: lenguajeGB.smsCreApoyo(), sourceUrl: redesMenu.getRandom(), thumbnail: await(await fetch(KratosMenu.getRandom())).buffer() }}}
...
const isCommand2 = /^(cuenta(s)?oficiales?|Atroig|(cuentas|account)s?g?b|(ig)?Atro|(cuentasatro))$/i.test(command)  
const isCommand3 = /^(atro(bot)?groups?|grupos(ofc|deatrobot|gb)|grupogb|groupsgb|atrogroups?|grupos|group(ofc|sofc))$/i.test(command) 
const isCommand4 = /^(c(ó|o)digo|sc|git|script)$/i.test(command) 
const isCommand5 = /^(instalar(kratos)?bot|proceso(del)?bot|bot(install|proceso)|installbot)$/i.test(command) 
...
const isCommand8 = /^(info(kratos|bot)|informaci(ón|on)(kratos|bot))$/i.test(command) 
...
*╰𝄗𝄗𝄗𝄗𝄗⬣*`.trim()
await conn.sendFile(m.chat, kratosImg.getRandom(), 'kratos.jpg', estado, fkontak)
} catch (e) {
reportError(e)
}   
break
...
await conn.sendFile(m.chat, media, 'kratos.jpg', lenguajeGB.smsOfc1() + '\n\n' + str, fkontak)  
} catch (e) {
reportError(e)
}    
break
...
_⭔ ${grupo[8]}_\n
_⭔ ${grupo[9]}_`.trim()
await conn.sendFile(m.chat, kratosImg.getRandom(), 'kratos.jpg', str, fkontak)   
} catch (e) {
reportError(e)
}    
break
...
await conn.sendFile(m.chat, kratosImg.getRandom(), 'kratos.jpg', `*_${_package.homepage}_*\n\n` + lenguajeGB.smsPrivadoDonar(), fkontak)    
} catch (e) {
reportError(e)
}    
break
...
let replit = `*◎ R E P L I T*\n\nhttps://replit.com//github.com/clubatro/kratos-bot`
let heroku = `*◎ H E R O K U*\n\nhttps://heroku.com/deploy?template=https://github.com/clubatro/kratos-bot-Heroku`
...
_package = JSON.parse(await promises.readFile(join(__dirname, '../package.json')).catch(_ => ({}))) || {}
let pp = await conn.profilePictureUrl(`${global.conn.user.jid.split`@`[0]}` + '@s.whatsapp.net', 'image').catch(_ => kratosImg.getRandom())
let aa = `*_🍄 ${lenguajeGB.smsCreA()} ${taguser}, ${lenguajeGB.smsCreB()} ${_package.name} ${lenguajeGB.smsCreC()}_*\n\n` 
let bb = `${packname}\n*◎ Wa.me/${owner[0][0]}*\n\n`
await conn.sendFile(m.chat, pp, 'kratos.jpg', aa + bb + ig, fkontak) 
} catch (e) {
reportError(e)
}    
break
...
await conn.sendFile(m.chat, kratosImg.getRandom(), 'kratos.jpg', info, fkontak) 
} catch (e) {
reportError(e)
}     
break

//CÓDIGO CREADO GRACIAS A https://github.com/Azami19 & https://github.com/clubatro/kratos-bot
case isCommand9:
try{
...
description = official[0][0] == String(contact[0]) ? 'Solo temas de KratosBot' : official[1][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[2][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[3][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : desc === '' ? lenguajeGB.smsContacto5() : desc
correo = official[0][0] == String(contact[0]) ? 'socialplus.clubatroofc@gamil.com' : official[1][0] == String(contact[0]) ? 'dced9092@gmail.com' : official[2][0] == String(contact[0]) ? 'ed3095@gmail.com' : mail === '' ? lenguajeGB.smsContacto6() : mail
...
enlace = official[0][0] == String(contact[0]) ? 'https://github.com/clubatro/kratos-bot' : official[1][0] == String(contact[0]) ? '' : official[2][0] == String(contact[0]) ? '' : official[3][0] == String(contact[0]) ? '' : md    
...
lista.push([conn.user.jid.split('@')[0], await conn.getName(conn.user.jid), packname, lenguajeGB.smsContacto8(), mail === '' ? '@gmail.com' : mail, lenguajeGB.smsContacto7(), md, bioBot, yt, ig, fb, paypal, nna])
...
await conn.sendFile(m.chat, kratosImg.getRandom(), 'Kratos.jpg', caption.trim(), fkontak) 
} catch (e) {
reportError(e)
}   
break
...
await conn.sendFile(m.chat, img21, 'Kratos.jpg', lenguajeGB.smsTituloDonar() + '\n\n' + donar + '\n\n' + paypal + '\n' + ig, fkontak) 
...
handler.command = /^(estado|status|estate|state|stado|stats|botstat(us)?|cuenta(s)?oficiales?|atroofcig|(cuentas|account)s?g?b|(ig)?Atro|(cuentasatro))|Kratos(bot)?groups?|grupos|groupsgb|grupos(ofc|dekratosbot|gb)|grupogb|Kratosgroups?|group(ofc|sofc)|c(ó|o)digo|sc|git|script|instalar(kratos)?bot|proceso(del)?bot|bot(install|proceso)|installbot|owner|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador|group(s|list|o(lista)?)|list(a)?(de)?grupo(s)?|grupolista|info(kratos|bot)|informaci(ón|on)(kratos|bot)|contactos?|contacts?|ping|speed|velocidad|rapidez|velocity|dona(te|si)|donar|apoyar|paypal|donating|aportar|report|request|reporte|bugs|bug|reportowner|reportes|reportar$/i
